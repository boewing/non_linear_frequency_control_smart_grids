...
for k=1:iterations
    ...
    %min|d-nabla_f(x)|^2
    %analytic version without constraints: d = n_f(x,mygrid)' - n_h(x,mygrid)'*inv(n_h(x,mygrid)*n_h(x,mygrid)')*n_h(x,mygrid)*n_f(x,mygrid)';
    ...
    assert(max(Aeq*d) < 1e-10, 'step is not in the tangent plane');
    
    %make a step
    x.setx(x.getx() + alpha*d);
    
    %retraction
    assert(x.theta(1)==0);              %check that the angle reference stays 0
    x = retraction(x, mygrid);
    assert(max(abs(h(x,mygrid))) < 1e-5, 'retraction failed');    %check that the retraction works makes h(x)=0
...
%sum:  3*n+2*m vars
    ...
    function newstate = retraction(mystate, mygrid)
        ...
        new_x_var = fsolve(@(y) h_fix(mystate, mygrid, y), initial_x_var,optimoptions('fsolve','Display','None','FunctionTolerance',1e-8));
