...
for k=1:iterations
    ...
    assert(abs(x.f) < 0.2,'massive frequency deviation: maybe not sufficient generation available');
    ...
    assert(abs(abs(det(T_small)) - abs(det(T))) < 1e-2, 'matrix reduction was wrong');
    %min_eig = min(abs(eig(T));
